...
import {
  ...
  moveColumnAction,
  reorderColumnAction,
  setColumnGlpiStatusAction,
  type GlpiHit,
} from "@/app/(marketing)/marketing/quadro/actions";

// Status do GLPI que uma coluna pode representar. 3 ("planejado") não aparece:
// mora junto do 2, e o servidor normaliza. Ver `normalizeGlpiStatus` em board.ts.
const GLPI_STATUS_OPTS: { id: number; label: string }[] = [
  { id: 1, label: "Novo" },
...
// Só concluir precisa de dado extra: a solução. Os outros status a API v1 grava
// direto. Espelha `statusPrecisaDe` do servidor.
const statusExige = (statusId: number): "nada" | "solucao" => (statusId === 5 ? "solucao" : "nada");

type StatusPrompt = { cardId: string; glpiId: number; toColumnId: string; index: number; colName: string };
...
export function QuadroBoard({ board, openCardId = null }: { board: BoardDTO; openCardId?: string | null }) {
  ...
  function drop(toColumnId: string, endIndex: number) {
    ...
    // Concluir registra a SOLUÇÃO no GLPI (é ela que leva o chamado a Solucionado
    // e fica na linha do tempo), então pede o texto antes de mover.
    if (escreveGlpi && statusExige(alvo!) === "solucao") {
      setStatusPrompt({ cardId: d.id, glpiId: card!.glpiId!, toColumnId, index, colName: toCol!.name });
      return;
    }
  ...
  return (
    <>
      ...
      <div className="row gap8" style={{ flexWrap: "wrap", alignItems: "center", marginBottom: "var(--gap)" }}>
        ...
        {/* Concluídas somem do quadro depois de 2 dias — este botão traz de volta. */}
        {(board.hiddenDone > 0 || board.showingDone) && (
          <button
            className="btn btn-ghost"
            style={{ marginLeft: "auto", padding: "3px 10px", fontSize: 12 }}
            onClick={() => router.push(board.showingDone ? "/marketing/quadro" : "/marketing/quadro?concluidas=1")}
          >
            <Icon name={board.showingDone ? "check" : "checkCircle"} size={14} />{" "}
            {board.showingDone ? "Ocultar concluídas" : `Ver concluídas (${board.hiddenDone})`}
          </button>
        )}
      </div>

      {board.showingDone && (
        <div className="card card-pad muted" style={{ marginBottom: "var(--gap)", fontSize: 12.5 }}>
          Mostrando também as concluídas há mais de {board.doneAfterDays} dias.
        </div>
      )}
          ...
          return (
            ...
            >
              <ColumnHeader
                ...
                onMapStatus={(statusId) =>
                  run(
                    () => setColumnGlpiStatusAction(col.id, statusId),
                    statusId == null ? "Coluna desvinculada do GLPI" : `Coluna = ${GLPI_STATUS_LABEL[statusId]} no GLPI`,
                  )
                }
...
// ── Concluir: pede a solução ─────────────────────────────────────────────────
// A v1 até aceitaria marcar "Solucionado" direto, mas aí o chamado ficaria
// resolvido SEM registro do que foi feito. Registrar a solução leva o status a 5
// sozinho e ainda deixa o histórico — por isso o texto é pedido aqui.
function StatusPromptModal({
  ...
  prompt: StatusPrompt;
  onCancel: () => void;
  onConfirm: (extra: { solucao: string }) => void;
}) {
    ...
    <Modal title={`Mover para ${prompt.colName}`} onClose={onCancel} maxWidth={470}>
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <p className="muted" style={{ margin: 0, fontSize: 12.5, lineHeight: 1.5 }}>
          No GLPI o chamado #{prompt.glpiId} é dado como solucionado ao receber uma solução.
        </p>
        <div>
        ...
        <div className="row gap8" style={{ justifyContent: "flex-end", marginTop: 4 }}>
          ...
          <button className="btn btn-primary" disabled={!solucao.trim()} onClick={() => onConfirm({ solucao })}>
            Concluir e mover
          </button>
        </div>
      </div>
...
function ColumnHeader({
  ...
  onMove: (dir: -1 | 1) => void;
  onDelete: () => void;
  onMapStatus: (statusId: number | null) => void;
  onDragStart: () => void;
  onDragEnd: () => void;
}) {
  ...
  return (
    ...
    >
      <span style={{ display: "flex", flexDirection: "column", gap: 2, minWidth: 0 }}>
        ...
        {glpiStatusId != null && (
          <span className="muted" style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: 0.3 }} title="Soltar um cartão de chamado aqui muda o status no GLPI">
            <Icon name="inbox" size={10} /> GLPI: {GLPI_STATUS_LABEL[glpiStatusId] ?? "?"}
          </span>
        )}
      </span>
      ...
      {menu && (
        <>
          ...
          <div style={{ position: "absolute", top: "100%", right: 8, zIndex: 41, background: "var(--surface, #fff)", border: "1px solid var(--line)", borderRadius: "var(--r-md)", boxShadow: "var(--shadow-md, 0 6px 20px rgba(0,0,0,.15))", minWidth: 160, overflow: "hidden" }}>
            ...
            <MenuItem label="Status do GLPI…" icon="inbox" onClick={() => { setMenu(false); setMapping(true); }} />
      ...
      {mapping && (
        <>
          <div onClick={() => setMapping(false)} style={{ position: "fixed", inset: 0, zIndex: 40 }} />
          <div style={{ position: "absolute", top: "100%", right: 8, zIndex: 41, background: "var(--surface, #fff)", border: "1px solid var(--line)", borderRadius: "var(--r-md)", boxShadow: "var(--shadow-md, 0 6px 20px rgba(0,0,0,.15))", minWidth: 210, overflow: "hidden" }}>
            <div className="muted" style={{ fontSize: 11, padding: "9px 12px 6px", fontWeight: 700 }}>
              Chamado solto aqui vira:
            </div>
            <MenuItem
              label={glpiStatusId == null ? "• Só do time (nenhum)" : "Só do time (nenhum)"}
              icon="kanban"
              onClick={() => { setMapping(false); onMapStatus(null); }}
            />
            {GLPI_STATUS_OPTS.map((s) => (
              <MenuItem
                key={s.id}
                label={glpiStatusId === s.id ? `• ${s.label}` : s.label}
                icon="inbox"
                onClick={() => { setMapping(false); onMapStatus(s.id); }}
              />
            ))}
          </div>
        </>
